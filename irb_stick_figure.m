...
% Run your symbolic kinematics definitions
run('irb1200_ForwardKinematics.m');

% Define a home configuration (all joint angles 0 degrees)
joint_angles_deg = [0, 0, 0, 0, 0, 0];
joint_angles_rad = deg2rad(joint_angles_deg);

% Define link lengths in meters (as from image)
...
ylabel('Y [m]');
zlabel('Z [m]');
title('IRB 1200 Stick Figure');

% Label joints
for i = 1:7
    text(origins(1, i), origins(2, i), origins(3, i), sprintf('  J%d', i-1));
end
